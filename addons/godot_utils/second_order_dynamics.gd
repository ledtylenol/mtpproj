...
# more complicated than the non-q one unfortunately
func update_q(current: Quaternion, target: Quaternion, delta: float) -> Array:
	_calc_k()

	if delta <= 0.0:
		return [current.normalized(), ydq]

	# keep target continuous relative to where it was last frame,
	# so the finite-difference velocity (xd) never spikes across
	# the +q/-q double-cover boundary

	# done because otherwise it shits the bed and picks the long-way-round sometimes
	if target.dot(xpq) < 0.0:
		target = -target

	# keep current in the same hemisphere as target so the
	# integration doesn't cross the double-cover seam either
	if current.dot(target) < 0.0:
		current = -current

	var xd: Quaternion = (target - xpq) / delta
	xpq = target

	var iterations: int = max(1, ceili(delta / _delta_crit))
...
func update_f(current: float, target: float, delta: float, use_angle = false) -> float:
	...
	var xd: float
	if use_angle:
		xd = wrapf(target - xpf, -PI, PI) / delta
	else:
		xd = (target - xpf) / delta
